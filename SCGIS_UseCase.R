...
#                          1.  Setup Environment                               #
################################################################################
in_dir <- 'D:/Downloads/SCGIS_Workshop/'       # IMPORTANT: Update to local dir
...
NDSI_stats <- aggregate(Date ~ TrailCam, data=NDSI, FUN=function(x) LastSnow=max(x))
...
CPUE$NDSI_Count <- NDSI_stats$Date # Add NDSI stats to the CPUE df
...
plot(CPUE$NDSI_Count ~ CPUE$CPUE, main="2015 Last Snow vs. Deer CPUE", ylab="Last Occurrence of Snow (DOY)",
